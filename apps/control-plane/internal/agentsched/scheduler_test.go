...
func (f *fakeTasks) CreateTask(ctx context.Context, tenantID, userID uuid.UUID, pack agenttask.Pack, instructions string, projectID uuid.UUID, attachments []agenttask.Attachment, bearerJWT string) (agenttask.Task, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.calls++
