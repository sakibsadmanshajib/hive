...
// The requested-account header must not select an invited membership either.
func TestEnsureViewerContext_RequestedInvitedAccountIsRefused(t *testing.T) {
	repo := newStubRepo()
	activeAccountID, invitedAccountID := uuid.New(), uuid.New()
	viewer := auth.Viewer{
		UserID:        uuid.New(),
	...
	for _, seed := range []struct {
		...
		{activeAccountID, "own-workspace", accounts.StatusActive, accounts.RoleOwner},
		{invitedAccountID, "pending-workspace", accounts.StatusInvited, accounts.RoleOwner},
	} {
		repo.accountsMap[seed.id] = &accounts.Account{
			ID:          seed.id,
			Slug:        seed.slug,
	...
	vc, err := accounts.NewService(repo).EnsureViewerContext(context.Background(), viewer, invitedAccountID)
	if err != nil {
		t.Fatalf("EnsureViewerContext error: %v", err)
	}
	if vc.CurrentAccount.ID != activeAccountID {
		t.Fatalf("current account = %s, want the active membership %s", vc.CurrentAccount.ID, activeAccountID)
	}
}
