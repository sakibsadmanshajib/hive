...
describe("RuntimeServices", () => {
    ...
    it("exposes openrouter/free when OpenRouter has an explicit free offer", async () => {
        vi.resetModules();
        vi.spyOn(console, "warn").mockImplementation(() => undefined);
        ...
        expect(services.models.list().map((model) => model.id)).toContain("openrouter/free");
        expect(services.models.list().map((model) => model.id)).toContain("openrouter/auto");
    });

    it("does not expose openrouter/free when OpenRouter has no free offer", async () => {
        vi.resetModules();
        vi.spyOn(console, "warn").mockImplementation(() => undefined);
        ...
        expect(services.models.list().map((model) => model.id)).not.toContain("openrouter/free");
    });
