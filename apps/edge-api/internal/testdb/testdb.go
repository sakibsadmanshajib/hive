...
// test is indistinguishable from a passing one inside a green check (issues
// #659 and #708). Centralising the gate means the rule "a missing DSN in CI
// is a red build, not a quiet skip" is stated once and cannot drift back
// package by package.
//
// Deliberately duplicated from apps/control-plane/internal/testdb: the two
// are separate Go modules under go.work, and an internal package cannot be
// imported across that boundary. Sixty lines of duplication beats a shared
// module that exists only to hold them.
package testdb
