...
// AccountingAdapter adapts *inference.AccountingClient to the audio.AccountingInterface.
type AccountingAdapter struct {
	inner *inference.AccountingClient
}

// NewAccountingAdapter wraps an AccountingClient for use with the audio Handler.
func NewAccountingAdapter(inner *inference.AccountingClient) *AccountingAdapter {
	return &AccountingAdapter{inner: inner}
}

// CreateReservation reserves credits for an audio request.
