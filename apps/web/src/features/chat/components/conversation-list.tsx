...
export function ConversationList({ conversations, activeConversationId, onNewChat, onSelectConversation }: ConversationListProps) {
  return (
    <div className="flex h-full flex-col gap-3">
      <Button type="button" onClick={onNewChat} className="justify-start rounded-xl">
        <Plus className="h-4 w-4" />
        New chat
      </Button>
      <p className="px-1 text-xs uppercase tracking-[0.12em] text-muted-foreground">Recent</p>
