<script lang="ts">
	...
	const submitCoworkRun = async (userPrompt: string) => {
		...
		if (_chatId === '') {
			...
			/*
			 * The run's row gets the brief as its title. Titles are normally
			 * generated by a follow-up completion the run path does not make, so
			 * without this every run sits in the conversation list as "New Chat"
			 * and the list stops being readable the moment there are two of them.
			 * The first line, trimmed: a task brief's opening sentence is what the
			 * person would have called it.
			 */
			const title = userPrompt.trim().split('\n')[0].slice(0, 80) || $i18n.t('Task');
			if (!$temporaryChatEnabled) {
			...
			<PaneGroup direction="horizontal" class="w-full h-full">
				<Pane defaultSize={50} minSize={30} class="h-full flex relative max-w-full flex-col">
					...
					<div id="chat-pane" class="flex flex-col flex-auto z-10 w-full @container overflow-auto">
						<!--
							hive: Hive has one home, and an upstream personalisation toggle
							does not get to decide whether it exists. `landingPageMode`
							used to be part of this condition, so an account that had ever
							set it to 'chat' skipped Placeholder.svelte entirely and landed
							on upstream's ChatPlaceholder: no greeting, no quick-start
							chips, just the model name over a placeholder string. Both of
							those shipped in #1161 and were invisible on any such account,
							which is a setting quietly deleting a feature. Messages exist or
							they do not; that is the only thing this branch decides now.
						-->
