...
HIVE PATCH NOTICE (issue #305/#308, security spike #307): this vendored copy
enforces --pid/--containall unconditionally and refuses to construct a bind
mount that references the Docker socket. See vendor/openhands/VENDORING.md
for the full list of patches applied to the upstream source. Hive's own
launcher (apps/agent-engine/internal/sandbox) does not call this class
directly; it independently constructs and validates the same `apptainer run`
...
class ApptainerWorkspace(RemoteWorkspace):
    ...
    def _start_container(self) -> None:
        ...
        for bind_spec in [*self.extra_bind_mounts, *env_extra_binds]:
            # HIVE PATCH (security spike #307, rows 8/9): a bind mount that
            # reaches the Docker socket must never be constructible, in the
            # vendored copy or upstream. Fails closed rather than silently
            # dropping the bind.
            lowered = bind_spec.lower()
            if "docker.sock" in lowered or "docker_host" in lowered:
                raise RuntimeError(
                    "HIVE PATCH: refusing Apptainer bind mount referencing "
                    f"the Docker socket: {bind_spec!r}"
                )
            bind_args += ["--bind", bind_spec]
            logger.info("Adding Apptainer bind mount: %s", bind_spec)

        # Build container options
        # HIVE PATCH (security spike #307, implementation condition 1):
        # --pid and --containall are unconditional, not configurable. The
        # upstream default (no --pid) shares the host PID namespace, which
        # the spike proved lets a sandboxed process observe host PIDs.
