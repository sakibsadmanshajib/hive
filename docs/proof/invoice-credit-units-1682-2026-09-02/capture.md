...
`InvoiceRow`, the real formatters, the real stylesheet. The upstream is stubbed,
...
nothing below the auth plumbing was altered. The figures, the columns, the
formatting and the absence handling are the shipped code's.
...
## 1. Before, the row as it stands today

The August 2026 row holds 524,653,338 in `total_bdt_subunits`, which is a Hive
credit count, and `formatTakaSubunits` divides it by one hundred.

```
PERIOD                      HIVE CREDITS   CHARGED          MODELS    DOWNLOAD
2026-08-01 -> 2026-09-01    —              ৳5,246,533.38    1 model   Download PDF
2026-07-01 -> 2026-08-01    —              ৳31.20           1 model   Download PDF
```
...
The stored figure is now taka, the credit quantity is on the row in its own
column, and the two reconcile: 524,653,338 credits is 0.524653338 USD at
1 USD = 1,000,000,000 credits (D-046), which is 64.60 BDT at the platform rate
of 123.13.

```
PERIOD                      HIVE CREDITS   CHARGED   MODELS    DOWNLOAD
2026-08-01 -> 2026-09-01    524,653,338    ৳64.60    1 model   Download PDF
2026-07-01 -> 2026-08-01    —              ৳31.20    1 model   Download PDF
```

Screenshot: `proof-03-console-after.png`.

The July row is the second case on purpose. It was generated after the #1648
fix, so its taka is correct and it never recorded a credit quantity. It renders
as an em dash, not as a zero, because a month of no consumption and an
unrecorded quantity are different claims and only one of them is true.

## 3. The regenerated PDF

Rendered by the production renderer on the repaired invoice. Both figures appear,
in separate columns, with the unit stated underneath.

```
Model       Requests   Hive credits   Charged (BDT)
hive-fast   412        524,653,338    BDT 64.60
                       ---------------------------
Total                  524,653,338    BDT 64.60

Consumption is metered in Hive credits. The BDT column is the amount charged for it.
```

Screenshot: `proof-02-invoice-pdf-regenerated.png`.
...
--- PASS: TestRepairUnconvertedInvoices_Live (0.11s)
=== RUN   TestUpdateConverted_RefusesAnAlreadyConvertedRow_Live
--- PASS: TestUpdateConverted_RefusesAnAlreadyConvertedRow_Live (0.07s)
=== RUN   TestRepairLeavesConvertedRowsUntouched_Live
--- PASS: TestRepairLeavesConvertedRowsUntouched_Live (0.07s)
```

Before: `total_bdt_subunits=524653338`, `total_credits=NULL`, `usd_bdt_rate=NULL`.
After: `total_bdt_subunits=5247`, `total_credits=524653338`, `usd_bdt_rate=100.000000`,
`usd_bdt_rate_source=env`. The 5,247 paisa figure is 52.47 taka, which is the
same 0.52 USD of consumption at the pinned test rate rather than the deployment
rate used in the screenshots.
