...
| `12-invoice-total-after-bengali.png` / `13-console-overview-after-bengali.png` | Re-verification after this branch was rebased onto the Bengali i18n work, which put Noto Sans Bengali behind Geist in the mono stack and rewrote the invoice row. Captured with the console in Bengali. Latin digits still come from Geist Mono, so every zero keeps its slash, and the credits caption and invoice total both hold. |
