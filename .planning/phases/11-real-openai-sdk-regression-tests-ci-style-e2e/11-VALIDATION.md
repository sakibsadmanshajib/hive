...
|---------|------|------|-------------|-----------|-------------------|-------------|--------|
| T1 | 01 | 1 | CI-04 | cast-free type surface + compile | `cd /home/sakib/hive && pnpm --filter @hive/api exec tsc --noEmit` | ✅ | ✅ green |
...
| T4 | 01 | 1 | CI-03 | SDK error-path regression (402, 429, 422) | `cd /home/sakib/hive && pnpm --filter @hive/api exec vitest run test/openai-sdk-regression.test.ts` | ✅ | ✅ green |
...
**Approval:** audited 2026-03-22 — cast-free compile surface, SDK regression suite (including 422), and full API suite green
...
All requirements are COVERED. This audit corrected two stale validation claims from the earlier artifact: `CI-03` now has a real 422 regression expectation (`OpenAI.UnprocessableEntityError`), and `CI-04` no longer relies on unsafe test/helper casts. No remaining manual-only gaps. `nyquist_compliant: true`.
