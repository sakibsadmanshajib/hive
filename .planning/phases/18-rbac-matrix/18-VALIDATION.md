...
- [ ] `apps/control-plane/internal/authz/permissions.go` — typed `Permission` const + registry with `RequiresVerified` flag.
- [ ] `apps/control-plane/internal/authz/policy.go` — `Actor` struct, `Policy.Can(actor, perm) bool`, `RequirePermission(perm)` middleware.
- [ ] `apps/control-plane/internal/authz/policy_test.go` — table-driven matrix test (the spec).
- [ ] `apps/control-plane/cmd/gen-permissions/main.go` — TS emitter.
- [ ] `apps/web-console/lib/control-plane/permissions.generated.ts` — codegen output (committed).
- [ ] `apps/web-console/lib/control-plane/permissions.parity.test.ts` — vitest parity test consuming a fixture both producers agree on.
- [ ] `apps/web-console/tests/e2e/rbac-unverified.spec.ts` — Playwright spec.
- [ ] `apps/control-plane/internal/authz/authz_integration_test.go` — handler-level role × verified × module coverage.
- [ ] `packages/openai-contract/scripts/lint-no-bare-role-check.mjs` — ripgrep wrapper, allowlist `internal/authz/`, `platform/role*.go`, matrix tests.
- [ ] `.github/workflows/*.yml` — add `make gen-permissions && git diff --exit-code` + `node packages/openai-contract/scripts/lint-no-bare-role-check.mjs` as required CI steps.
- [ ] Makefile target `gen-permissions` that wraps the emitter.
