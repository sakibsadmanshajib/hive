# DEPRECATED - LibreChat Upgrade Playbook (Hive chat-app)

> Superseded on 2026-05-17 by the Open WebUI pivot. Hive Chat now consumes the upstream Open WebUI image through env, pipeline filters, and Caddy; it does not vendor a LibreChat fork. Keep this file only as historical context for the rejected v3 plan.

**Owner:** chat-app maintainers (Phase 19 → Phase 25 + ongoing).
**Vendored at:** `apps/chat-app/` (no nested `.git`).
**Pin:** see [`LIBRECHAT-VERSION.DEPRECATED.md`](./LIBRECHAT-VERSION.DEPRECATED.md).
...
- Update [`LIBRECHAT-VERSION.DEPRECATED.md`](./LIBRECHAT-VERSION.DEPRECATED.md) — new `pinned_tag`, new `commit_sha`, new `forked_at`.
- Update `LIBRECHAT-UPGRADE-PLAYBOOK.DEPRECATED.md` if any healthcheck path / build script / interface key changed.
...
- Pinned-tag history: [`LIBRECHAT-VERSION.DEPRECATED.md`](./LIBRECHAT-VERSION.DEPRECATED.md)
