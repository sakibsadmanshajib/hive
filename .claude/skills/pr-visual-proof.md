...
`docs/proof/<slug>/` in the same PR: `npm run lint:proof-tokens` scans that
directory and nothing else, so a log left in a scratch dir or a comment is
unscanned and the check reports green over the old corpus instead of failing.
Nothing scans the upload itself either. A release asset is blob storage on a
tag, not a git object, so GitHub secret scanning and GitGuardian never see it.
