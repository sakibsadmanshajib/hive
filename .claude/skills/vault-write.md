...
A global skill, `~/.claude/skills/obsidian-second-brain/SKILL.md`, covers the
same vault-write policy generically across every project. Use this skill
instead of, not in addition to, that one when working in hive specifically:
this one adds what's hive-only and load-bearing for the other three skills in
this set: never silently overwriting an existing doc (Step 1), the real
six-section MOC-plans.md structure and content-based section picking (Step
5), and syncing `.wolf/decisions.md` for decision docs (Step 4), none of
which the global skill knows about since it isn't hive-specific. Don't edit
the global skill from here; it's out of this repo's scope.
...
`MOC-plans.md` has six section tables: Decisions, Plans, Specs, Research,
Discussions, Archived. Pick by what the document actually is, not a rigid
mapping off its `type:` frontmatter: the two don't line up one-to-one in
practice (an `architecture`-type redesign spec is filed under Specs; an
`architecture`-type audit doc is filed under Research because it reads as an
investigation, not a design). As a starting point: `decision` → Decisions,
`plan` → Plans, `spec` or a design-shaped `architecture` doc → Specs, an
investigation/audit-shaped doc (any type) → Research, `discussion` (including
session-narrative docs) → Discussions. Never invent a seventh section or skip
the step because your doc's type doesn't obviously match one, pick the
closest fit by content and move on.

Add a row to that table with a `[[wikilink]]`, the date, and a one-paragraph
summary in the vault's existing voice: dense, specific, naming the concrete
trigger and the concrete outcome, match the density of the rows already
there, not a one-line stub. This step is not optional: `MOC-plans.md` is the
first place `vault-search` looks, and a document left off it is effectively
