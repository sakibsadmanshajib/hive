...
|------|-------------------|---------------|
| **claude-mem** | Cross-session observation store w/ search (`mem-search`, `get_observations`, `timeline`, `smart_search`, `smart_outline`, `smart_unfold`) | `mcp__plugin_claude-mem_mcp-search__*` tools available, or `claude-mem` skill listed |
