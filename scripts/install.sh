...
patch_sovereign_config() {
    ...
    # Extract any Ollama model entries that patch_ollama_config already inserted.
    # These are lines between the first `model_list:` and `litellm_settings:` that
    # reference `ollama` (model_name, model, api_base lines for ollama routes).
    # We rebuild a clean config containing only those entries plus the global
    # litellm_settings block (with telemetry: False already set).
    _ollama_block="$(awk '/^model_list:/{found=1; next} found && /^litellm_settings:/{exit} found && /ollama/{print; next} found && /^  -/{buf=$0; next} found && buf!="" && /ollama/{print buf; buf=""; print; next} found{buf=""}' "$LITELLM_CONFIG" || true)"
...
# Cloud routes (OpenRouter, Groq) have been removed to enforce zero external
# LLM egress. Only local Ollama models are available.
model_list:
${_ollama_block}
litellm_settings:
  telemetry: False
